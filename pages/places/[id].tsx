...
const Place: NextPageWithAuth = () => {
  const router = useRouter();
  const { id } = router.query;
  const { data: place, error } = useSWR<Place>(
    id && "/api/places/place?id=" + id,
    fetcher
  );
    ...
    place && (
      <div className="flex flex-col items-center">
        <div className="flex flex-col items-center max-w-max mx-auto">
          <p className="text-xl md:text-2xl font-semibold mt-12 mb-2 border-b-[3px] pb-1 px-2 border-white ">
            {place.name}, {place.country}
          </p>
          <p className="mb-4 self-start">{place.date}</p>
        </div>
        <div className="w-max max-w-[80vw] md:max-w-[60vw] mx-auto">
          <p className="self-start">{place.description}</p>
        </div>
